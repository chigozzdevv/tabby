    ...
    async function load() {
      try {
        ...
        const [next, evts] = await Promise.all([
          fetchJson<PublicGasLoanNextDue | null>(urls.nextDueUrl),
          fetchJson<ActivityEvent[]>(urls.eventsUrl),
        ]);
        if (cancelled) return;
        setNextDue(next);
            ...
            {borrower ? (
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-neutral-950/50 p-4 text-sm text-neutral-300">
                ...
                <div className="mt-2 text-xs text-neutral-400">
                  Open the dashboard link your agent posts in Telegram (it should include <span className="font-mono">borrower=</span>).
                </div>
              </div>
            )}
          </section>
