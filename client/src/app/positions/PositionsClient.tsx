...
const formatAmount = (value: string | undefined, decimals: number, unit: string) => {
  const numberPart = formatUnitsCompact(value, decimals);
...
export default function PositionsClient() {
  ...
  const handleSecuredWithdraw = async () => {
    ...
    await runTx("Secured withdraw", async () => {
      const { walletClient, account } = await getWalletClient();
      await ensureTargetChain(walletClient);
      const shareDecimals = securedPool.assetDecimals ?? 18;
      const hash = await walletClient.writeContract({
        address: securedPool.address as Address,
        abi: securedPoolAbi,
        functionName: "withdraw",
  ...
  const handleSecuredStake = async () => {
    ...
    await runTx("Stake secured shares", async () => {
      const { walletClient, account } = await getWalletClient();
      await ensureTargetChain(walletClient);
      const shareDecimals = poolData?.secured?.assetDecimals ?? 18;
      const hash = await walletClient.writeContract({
        address: rewardsAddress as Address,
        abi: rewardsAbi,
        functionName: "stake",
  ...
  const securedDecimals = poolData?.secured?.assetDecimals ?? 18;
  const securedSymbol = poolData?.secured?.assetSymbol ?? "WMON";
  const securedDepositWei = parseUnitsOrNull(securedDeposit.trim(), securedDecimals);
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData?.native?.totalAssetsWei, 18, "MON")}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Pool balance</p>
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData?.native?.poolBalanceWei, 18, "MON")}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Outstanding</p>
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData?.native?.totalOutstandingPrincipalWei, 18, "MON")}
                  </p>
                </div>
                <div>
            ...
            {loadingPools ? (
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Total assets</p>
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData.secured.totalAssetsWei, securedDecimals, securedSymbol)}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Pool balance</p>
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData.secured.poolBalanceWei, securedDecimals, securedSymbol)}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Outstanding</p>
                  <p className="mt-2 text-sm font-semibold text-white">
                    {formatAmount(poolData.secured.totalOutstandingPrincipalWei, securedDecimals, securedSymbol)}
                  </p>
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Utilization</p>
                ...
                {positionNative ? (
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm font-semibold text-white">
                        {formatAmount(positionNative.estimatedAssetsWei, 18, "MON")}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm font-semibold text-white">
                        {formatAmount(rewards?.native?.earned, 18, "TABBY")}
                      </p>
                    </div>
                  </div>
                ) : (
                ...
                {positionSecured ? (
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Shares</p>
                      <p className="mt-2 text-sm font-semibold text-white">
                        {formatUnitsCompact(positionSecured.shares, securedDecimals)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Estimated assets</p>
                      <p className="mt-2 text-sm font-semibold text-white">
                        {formatAmount(positionSecured.estimatedAssetsWei, securedDecimals, securedSymbol)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Share of pool</p>
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Earned rewards</p>
                      <p className="mt-2 text-sm font-semibold text-white">
                        {formatAmount(rewards?.secured?.earned, 18, "TABBY")}
                      </p>
                    </div>
                  </div>
                ) : (
